...
void Simulation::initialize_matrix() {
  std::fill(&matrix[0][0], &matrix[0][0] + width * height, 0);
  std::fill(&new_matrix[0][0], &new_matrix[0][0] + width * height, 0);
}

void Simulation::execute_sim_step() {
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      if (get_from_coord(i, j) == 0) {
        continue;
      }
      ...
      if (!check_collision(i, j, types[get_from_coord(i, j)].move_to)) {
        ...
        int choice =
            choose_from_fork(i, j, types[get_from_coord(i, j)].or_else);
        if (set_from_coord(i + COL_POSITIONS[choice][0],
                           j + COL_POSITIONS[choice][1],
                           get_from_coord(i, j))) {
          set_from_coord(i, j, ParticleTypes::NONE);
        }
      }
...
bool Simulation::check_collision(int x, int y, int bit) {
  for (int i = 0; i < 8; i++) {
    if ((bit >> (7 - i) & 0x1) == 1) {
      if (get_from_coord(x + COL_POSITIONS[i][0], y + COL_POSITIONS[i][1]) !=
          0) {
        return true;
      }
    }
